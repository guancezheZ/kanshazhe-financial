    ...
    <el-card shadow="never">
      <el-tabs v-model="tab">
        <el-tab-pane label="客户档案" name="customers">
          <div style="margin-bottom:12px"><el-button size="small" type="primary" @click="showCust=true">新增客户</el-button></div>
        ...
        <el-tab-pane label="供应商档案" name="vendors">
          <div style="margin-bottom:12px"><el-button size="small" type="primary" @click="showVen=true">新增供应商</el-button></div>
...
function loadCust() {
  try { customers.value = JSON.parse(localStorage.getItem('jd_arap_customers') || '[]') } catch { customers.value = [] }
}
function loadVen() {
  try { vendors.value = JSON.parse(localStorage.getItem('jd_arap_vendors') || '[]') } catch { vendors.value = [] }
}
function saveCust() {
  if (!cf.name) return ElMessage.warning('请输入名称')
  const list = [...customers.value, { id: Date.now(), ...cf }]
  localStorage.setItem('jd_arap_customers', JSON.stringify(list))
...
function saveVen() {
  ...
  localStorage.setItem('jd_arap_vendors', JSON.stringify(list))
...
function delCust(row) {
  ElMessageBox.confirm('确认删除？').then(() => {
    ...
    localStorage.setItem('jd_arap_customers', JSON.stringify(list)); loadCust(); ElMessage.success('已删除')
  }).catch(() => {})
}
function delVen(row) {
  ElMessageBox.confirm('确认删除？').then(() => {
    const list = vendors.value.filter(v => v.id !== row.id)
    localStorage.setItem('jd_arap_vendors', JSON.stringify(list)); loadVen(); ElMessage.success('已删除')
  }).catch(() => {})
}
