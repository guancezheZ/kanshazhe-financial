...
function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log('  ⚠️  v0.8.3 安全加固：激活码必须 upload 到 Worker 才能使用！')
      console.log('  ═══════════════════════════════════════════════════════════\n')
      console.log('  命令:')
      console.log('    gen <数量>     生成激活码（默认1个）')
      console.log('    upload <令牌>   ⭐ 必做：将生成的码上传到 Worker（否则无法激活）')
      console.log('    export [令牌]   查看密钥状态（有令牌时从 Worker 取实时数据）')
      console.log('    verify <码>     验证激活码是否有效')
      console.log('    revoke <令牌> <码>  吊销指定激活码（从 Worker 彻底删除）')
      console.log('    clean <令牌>        清理已吊销的旧密钥')
      console.log('    sync <令牌>         从 Worker 同步激活状态到本地')
      console.log('    purge <令牌>        清理 Worker 上的无效密钥')
      console.log('    batch <文件.csv>    从CSV批量导入生成\n')
      console.log('  流程: gen → upload → 发给用户')
      console.log('  ──────────────────────────────────────────────────────────')
      console.log('    node scripts/key-manager.cjs gen 10           生成10个')
      console.log('    node scripts/key-manager.cjs upload <令牌>    上传到 Worker（必须！）')
      console.log('    node scripts/key-manager.cjs export <令牌>    查看云端状态')
      console.log('    node scripts/key-manager.cjs revoke <令牌> ABCD-1234  吊销')
      console.log('    node scripts/key-manager.cjs clean <令牌>  清理旧吊销密钥')
      console.log('    node scripts/key-manager.cjs sync <令牌>    同步状态')
      console.log('    node scripts/key-manager.cjs purge <令牌>   清理垃圾数据')
      console.log('    node scripts/key-manager.cjs verify ABCD    验证密钥')
      console.log('    node scripts/key-manager.cjs export         查看本地\n')
  }
}
